module make_new_grids_module
  ...
  public :: make_new_grids,make_new_grids_init
  public :: make_new_grids_den,make_new_grids_flw

  contains

  subroutine make_new_grids(new_grid_flag,la_crse,la_fine,flw,mf,dx_crse,buf_wid,ref_ratio, &
                              lev,max_grid_size,pfpara,aux_tag_mf)
       logical                 , intent(  out) :: new_grid_flag
       type(layout)            , intent(in   ) :: la_crse
       type(layout)            , intent(inout) :: la_fine
       type(multifab)          , intent(in   ) :: mf,flw
       real(dp_t)              , intent(in   ) :: dx_crse
       integer                 , intent(in   ) :: buf_wid,max_grid_size,ref_ratio,lev
       ...
       case(0)
         ...
         if(present(aux_tag_mf)) then
           call make_new_grids_den(new_grid_flag,la_crse,la_fine,mf,dx_crse,buf_wid,ref_ratio, &
                                   lev,max_grid_size,pfpara,aux_tag_mf)
         else
           call make_new_grids_den(new_grid_flag,la_crse,la_fine,mf,dx_crse,buf_wid,ref_ratio, &
                                   lev,max_grid_size,pfpara)
         end if

       case(1)
         
         if(present(aux_tag_mf)) then
           call make_new_grids_flw(new_grid_flag,la_crse,la_fine,flw,mf,dx_crse,buf_wid,ref_ratio, &
                              lev,max_grid_size,pfpara,aux_tag_mf)
         else
           call make_new_grids_flw(new_grid_flag,la_crse,la_fine,flw,mf,dx_crse,buf_wid,ref_ratio, &
                              lev,max_grid_size,pfpara)
  ...
  subroutine make_new_grids_init(new_grid_flag,la_crse,la_fine,flw,mf,seed_pos,pfpara,dx_crse,&
                                   buf_wid,ref_ratio,lev,max_grid_size,prob_lo,prob_hi,aux_tag_mf)
       type(multifab)          , intent(in   ) :: mf,flw
       logical                 , intent(  out) :: new_grid_flag
       type(layout)            , intent(in   ) :: la_crse
       type(layout)            , intent(inout) :: la_fine
       real(dp_t)              , intent(in   ) :: seed_pos(:,:)
       real(dp_t)              , intent(in   ) :: dx_crse
       real(dp_t)              , intent(in   ) :: prob_lo(mf%dim),prob_hi(mf%dim)
       integer                 , intent(in   ) :: buf_wid,max_grid_size,ref_ratio,lev
       type(pf_para)           , intent(in   ) :: pfpara
       type(multifab), optional, intent(in   ) :: aux_tag_mf
       ...
       case(0)
         ...
         if(present(aux_tag_mf)) then
           call make_new_grids_init_den(new_grid_flag,la_crse,la_fine,mf,seed_pos,pfpara,dx_crse,&
                                   buf_wid,ref_ratio,lev,max_grid_size,prob_lo,prob_hi,aux_tag_mf)
         else
           call make_new_grids_init_den(new_grid_flag,la_crse,la_fine,mf,seed_pos,pfpara,dx_crse,&
                                   buf_wid,ref_ratio,lev,max_grid_size,prob_lo,prob_hi)
         end if

       case(1)

         if(present(aux_tag_mf)) then
           call make_new_grids_init_flw(new_grid_flag,la_crse,la_fine,flw,mf,seed_pos,pfpara,dx_crse,&
                                   buf_wid,ref_ratio,lev,max_grid_size,prob_lo,prob_hi,aux_tag_mf)
         else
           call make_new_grids_init_flw(new_grid_flag,la_crse,la_fine,flw,mf,seed_pos,pfpara,dx_crse,&
                                   buf_wid,ref_ratio,lev,max_grid_size,prob_lo,prob_hi)
  ...
  end subroutine make_new_grids_init

  subroutine make_new_grids_den(new_grid_flag,la_crse,la_fine,mf,dx_crse,buf_wid,ref_ratio, &
                              lev,max_grid_size,pfpara,aux_tag_mf)
       ...
       if (present(aux_tag_mf)) then
          call make_boxes(mf,ba_new,dx_crse,buf_wid,lev,pfpara,aux_tag_mf)
       else
          call make_boxes(mf,ba_new,dx_crse,buf_wid,lev,pfpara)
       endif
  ...
  end subroutine make_new_grids_den

  subroutine make_new_grids_init_den(new_grid_flag,la_crse,la_fine,mf,seed_pos,pfpara,dx_crse,&
                                   buf_wid,ref_ratio,lev,max_grid_size,prob_lo,prob_hi,aux_tag_mf)
       ...
       if (present(aux_tag_mf)) then
          call make_boxes_init(mf,seed_pos,ba_new,dx_crse,buf_wid,lev,prob_lo,prob_hi,pfpara,aux_tag_mf)
       else
          call make_boxes_init(mf,seed_pos,ba_new,dx_crse,buf_wid,lev,prob_lo,prob_hi,pfpara)
       endif
  ...
  end subroutine make_new_grids_init_den

  subroutine make_new_grids_flw(new_grid_flag,la_crse,la_fine,flw,mf,dx_crse,buf_wid,&
                     ref_ratio,lev,max_grid_size,pfpara,aux_tag_mf)
       ...
       if (present(aux_tag_mf)) then
          call make_boxes_flw(flw,mf,ba_new,dx_crse,buf_wid,lev,pfpara,aux_tag_mf)
       else
          call make_boxes_flw(flw,mf,ba_new,dx_crse,buf_wid,lev,pfpara)
       endif
  ...
  end subroutine make_new_grids_flw

  subroutine make_new_grids_init_flw(new_grid_flag,la_crse,la_fine,flw,mf,seed_pos,pfpara,dx_crse,&
                                   buf_wid,ref_ratio,lev,max_grid_size,prob_lo,prob_hi,aux_tag_mf)
       ...
       if (present(aux_tag_mf)) then
          call make_boxes_init_flw(flw,mf,seed_pos,ba_new,dx_crse,buf_wid,lev,prob_lo,prob_hi,pfpara,aux_tag_mf)
       else
          call make_boxes_init_flw(flw,mf,seed_pos,ba_new,dx_crse,buf_wid,lev,prob_lo,prob_hi,pfpara)
       endif
  ...
  end subroutine make_new_grids_init_flw

  subroutine make_boxes_flw(flw,mf,ba_new,dx_crse,buf_wid,lev,pfpara,aux_tag_mf)
      ...
      if (present(aux_tag_mf)) then
         call tag_boxes_flw(flw,mf,tagboxes,dx_crse,llev,pfpara%para_uc,pfpara%para_th,pfpara%para_u,&
                            pfpara%amr_thers,pfpara%multi_add_tau,aux_tag_mf)
      else
         call tag_boxes_flw(flw,mf,tagboxes,dx_crse,llev,pfpara%para_uc,pfpara%para_th,pfpara%para_u,&
                            pfpara%amr_thers,pfpara%multi_add_tau)
      endif
  ...
  end subroutine make_boxes_flw

  subroutine make_boxes_init_flw(flw,mf,seed_pos,ba_new,dx_crse,buf_wid,lev,prob_lo,prob_hi,pfpara,aux_tag_mf)
      ...
      if (present(aux_tag_mf)) then
         call tag_boxes_init_flw(flw,mf,seed_pos,pfpara%seed_radius,pfpara%seed_num,tagboxes,&
                             dx_crse,prob_lo,prob_hi,pfpara%safe_n_init,pfpara%seed_type,&
                             pfpara%solve_flow_only,aux_tag_mf)
      else
         call tag_boxes_init_flw(flw,mf,seed_pos,pfpara%seed_radius,pfpara%seed_num,tagboxes,&
                             dx_crse,prob_lo,prob_hi,pfpara%safe_n_init,pfpara%seed_type,&
                             pfpara%solve_flow_only)
      endif
  ...
  end subroutine make_boxes_init_flw

  subroutine make_boxes(mf,ba_new,dx_crse,buf_wid,lev,pfpara,aux_tag_mf)
      ...
      if (present(aux_tag_mf)) then
         call tag_boxes(mf,tagboxes,dx_crse,llev,pfpara%para_uc,pfpara%para_th,&
                               pfpara%amr_thers,aux_tag_mf)
      else
         call tag_boxes(mf,tagboxes,dx_crse,llev,pfpara%para_uc,pfpara%para_th,&
                               pfpara%amr_thers)
      endif
  ...
  subroutine make_boxes_init(mf,seed_pos,ba_new,dx_crse,buf_wid,lev,prob_lo,prob_hi,pfpara,aux_tag_mf)
      ...
      if (present(aux_tag_mf)) then
         call tag_boxes_init(mf,seed_pos,pfpara%seed_radius,pfpara%seed_num,tagboxes,&
                             dx_crse,prob_lo,prob_hi,pfpara%safe_n_init,pfpara%seed_type,aux_tag_mf)
      else
         call tag_boxes_init(mf,seed_pos,pfpara%seed_radius,pfpara%seed_num,tagboxes,&
                             dx_crse,prob_lo,prob_hi,pfpara%safe_n_init,pfpara%seed_type)
      endif
  ...
  end subroutine make_boxes_init
