...
contains
  ...
  subroutine fabio_ml_multifab_read_d2(mla, mmf, mba,root, def_pmask,ng)
    ...
    logical, intent(in), optional   :: def_pmask(:)
    ...
    lpmask = .false.; if( present(def_pmask) ) lpmask = def_pmask
  ...
  contains
    ...
    subroutine build_ns_plotfile()
      ...
      type(layout) :: la
      type(layout), allocatable :: la_array(:)  ! newly added
      ...
      ! building the layout arrays
      allocate(la_array(flevel)) ! newly added

      do i = 1, flevel
         call build(la_array(i), balevs(i), boxarray_bbox(balevs(i)))  ! newly changed
         call build(mmf(i), la_array(i), nc = nvars, ng = ng, nodal = nodal(1:dm)) ! newly changed
      end do
      ...
      mba%pd(1) = bbox(get_boxarray(mmf(1))) ! newly added

      ! Build the boundary
      do i = 2, flevel   ! newly added
          mba%pd(i) = refine(mba%pd(i-1), mba%rr(i-1,1)) ! newly added
      end do

      call ml_layout_build_la_array(mla,la_array,mba,lpmask,flevel)
      !call build(mla,mba,la_array,lpmask,flevel) ! newly added

    end subroutine build_ns_plotfile

  end subroutine fabio_ml_multifab_read_d2
...
!    nl = size(mfs)
  ...
  subroutine fabio_ml_boxarray_read(mba,time, root)
    use bl_stream_module
    use bl_IO_module
    use bl_error_module
    type(ml_boxarray), intent(out) :: mba
    character(len=*), intent(in) :: root
    real(kind=dp_t),intent(out) :: time
    integer :: lun
