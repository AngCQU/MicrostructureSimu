module multifab_fill_ghost_module
  ...
  public ::  multifab_fill_ghost_cells, multifab_fill_ghost_cells_flw,multifab_fill_ghost_cells_t
...
subroutine multifab_fill_ghost_cells_flw(fine,crse,alfa,ng,ir,bc_crse,bc_fine,icomp,bcomp,nc, &
                                       stencil_width_input,fourth_order_input, &
                                       fill_crse_boundary_input,fill_crse_physbc_input)
    ...
    do i = 1, nfabs(ghost)
       ...
       select case(dm)
       case (2)
           call modify_f_values_flw_2d(dst(:,:,1,1),dst(:,:,1,2),dst(:,:,1,3),dst(:,:,1,4:12),alfa)
       case (3)
           call modify_f_values_flw_3d(dst(:,:,:,1),dst(:,:,:,2),dst(:,:,:,3),dst(:,:,:,4),&
                                       dst(:,:,:,5:23),alfa)
       end select
    end do
    !$OMP END PARALLEL DO
  ...
  end subroutine multifab_fill_ghost_cells_flw
