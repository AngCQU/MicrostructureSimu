module ml_restrict_fill_module
  ...
  public :: ml_restrict_and_fill,ml_restrict_and_fill_flw
...
contains
  ...
  subroutine ml_restrict_and_fill_flw(nlevs, mf, rr, bc,tau0, &
                                      icomp, bcomp, nc, ng, time_in,dx_in, prob_lo_in, prob_hi_in, &      
                                      stencil_width_input,fourth_order_input, same_boundary)
    ...
    do n=nlevs,2,-1
       ! calculate the alpha
       call cal_tau_alpha(alpha, n-1,tau0)
       call ml_cc_restriction_c_flw(mf(n-1), licomp, mf(n), licomp, rr(n-1,:), lnc, 1.d0/alpha)
    end do
    ...
    do n=2,nlevs
       ...
       if (lsameb) then
          do i=1, lnc
             call multifab_fill_ghost_cells_flw(mf(n), mf(n-1),alpha, lng, rr(n-1,:), bc(n-1), bc(n), &
                                            licomp+i-1, lbcomp, 1, &
                                            stencil_width_input      = stencil_width_input, &
                                            fourth_order_input       = fourth_order_input, &
                                            fill_crse_boundary_input = .false., &
  ...
  end subroutine ml_restrict_and_fill_flw
